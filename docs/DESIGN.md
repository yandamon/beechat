...
>
> 进度：2026-09-26 v1 范围内的功能已全部实现并有测试覆盖，v1.1 中的撤回、私聊已读回执、桌面通知也已完成；线上图片存储仍用本地驱动，等 R2 密钥填入后切换。
...
引用回复、消息回应、Web 推送、拉黑、举报、注销账号、置顶与免打扰、自定义群头像、简单自动回复机器人。已提前完成：私聊已读回执（`conversation:read` 推给整个会话）、两分钟内撤回（软删除并广播 `message:updated`，历史里保留占位）、桌面通知（页面不在前台或没开着该会话时，用 Notification API 提醒，需用户在侧栏打开）。
