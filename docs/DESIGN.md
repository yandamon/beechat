...
- 演示：固定演示账号一键登录，每日定时重置数据。
- PWA：可安装（manifest），不做离线阅读。

### v1.1

已读回执、撤回与删除、引用回复、消息回应、Web 推送、拉黑、举报、注销账号、置顶与免打扰、自定义群头像、简单自动回复机器人。
...
| `DATABASE_URL`          | Postgres 连接串                         | 数据库接入后   |
...
PostgreSQL，Drizzle 管理迁移。所有表带 `created_at`；外键全部带级联删除，为 v1.1 的注销账号预留。
...
5. 正在输入与在线状态：不落库，纯广播；输入事件客户端节流；断线后延迟 10 秒再标记离线，避免刷新页面时闪烁。
6. 连接建立时，服务端把用户加入其全部会话的房间，并订阅好友的在线状态。
